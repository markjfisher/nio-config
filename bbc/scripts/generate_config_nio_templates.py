...
def write_layout_header(layout: dict, header_lines: int) -> None:
    ...
    lines = [
        ...
        "",
        f"#define CONFIG_NIO_BBC_TEMPLATE_HEADER_LINES {header_lines}",
        f"#define CONFIG_NIO_BBC_TEMPLATE_HEADER_BYTES {header_lines * WIDTH}",
    ]
...
def write_layout_include(layout: dict, header_lines: int) -> None:
    out = SRC_BBC_DIR / "config_nio_layout.inc"
    lines = [
        "; Generated by bbc/scripts/generate_config_nio_templates.py",
        "",
        f"CONFIG_NIO_BBC_TEMPLATE_HEADER_LINES := {header_lines}",
        f"CONFIG_NIO_BBC_TEMPLATE_HEADER_BYTES := {header_lines * WIDTH}",
        "",
    ]
    ...
    def emit(prefix: tuple[str, ...], value) -> None:
        if isinstance(value, dict):
            ...
            lines.append(f"{macro_name(*prefix)} := {value}")
...
def write_template_data(assets: dict[str, bytes], header_lines: int) -> None:
    ...
    header_bytes = header_lines * WIDTH
    header = next(iter(assets.values()))[:header_bytes]
    if any(data[:header_bytes] != header for data in assets.values()):
        raise ValueError("template assets do not share the requested common header")

    blocks = [("_config_nio_tpl_header", header)]
    blocks.extend((labels[name], data[header_bytes:]) for name, data in assets.items())
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--header-lines", type=int, default=DEFAULT_HEADER_LINES,
                        help=f"number of common template header lines (default: {DEFAULT_HEADER_LINES})")
...
def main() -> int:
    ...
    if not 0 <= args.header_lines < HEIGHT:
        raise ValueError(f"--header-lines must be between 0 and {HEIGHT - 1}")
    assets = {
    ...
    write_template_data(assets, args.header_lines)
    write_layout_header(layout, args.header_lines)
    write_layout_include(layout, args.header_lines)
    return 0
