...
CFLAGS += --include-dir $(TARGET_INCLUDE_DIR)
...
LDFLAGS += -t $(TOOLCHAIN_TARGET)
ifeq ($(TARGET),atari)
LDFLAGS += --config $(ATARI_LINK_CONFIG) --start-addr $(ATARI_START_ADDRESS)
else ifneq ($(filter $(TARGET),bbc master bbc-clib),)
LDFLAGS += --start-addr $(BBC_START_ADDRESS)
EXTRA_LINK_OBJS += $(FUJINET_NIO_LIB)/obj/$(TOOLCHAIN_TARGET)/platform/bbc/initenv.o
endif

define compile_c
	$(CC) -t $(TOOLCHAIN_TARGET) -c --create-dep $(@:.o=.d) --listing $(@:.o=.lst) $(CFLAGS) -o $@ $<
endef
